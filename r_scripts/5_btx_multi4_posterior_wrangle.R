...
btx_study_sample_posterior <- btx_study_levels %>%
  mutate(N_study = purrr::map(data, ~posterior_samples(btx_multi4_fit, 
                                                       pars = paste('r_study_trt__N[', as.character(.x$level), ',cYear]', sep=''),
                                                       fixed = TRUE,
                                                       # want 1000 samples
                                                       subset = seq(1, 4000, by = 4))
                              %>% unlist() %>% as.numeric()),
         Sn_study = purrr::map(data, ~posterior_samples(btx_multi4_fit, 
                                                        pars = paste('r_study_trt__Sn[', as.character(.x$level), ',cYear]', sep=''),
                                                        fixed = TRUE,
                                                        # want 1000 samples
                                                        subset = seq(1, 4000, by = 4))
                               %>% unlist() %>% as.numeric()),
         S_study = purrr::map(data, ~posterior_samples(btx_multi4_fit, 
                                                       pars = paste('r_study_trt__S[', as.character(.x$level), ',cYear]', sep=''),
                                                       fixed = TRUE,
                                                       # want 1000 samples
                                                       subset = seq(1, 4000, by = 4))
                              %>% unlist() %>% as.numeric()),
         ENSPIE_study = purrr::map(data, ~posterior_samples(btx_multi4_fit, 
...
btx_study_summary <- btx_study_sample_posterior %>% 
  group_by(study_trt) %>% 
  summarise(N_slope = median(N_study),
            N_lower = quantile(N_study, probs = 0.05),
            N_upper = quantile(N_study, probs = 0.95),
            N_change = ifelse((N_upper > 0 & N_lower < 0), 0.9, 1),
            S_slope = median(S_study),
            S_lower = quantile(S_study, probs = 0.05),
            S_upper = quantile(S_study, probs = 0.95),
            S_change = ifelse((S_upper > 0 & S_lower < 0), 0.9, 1),
            Sn_slope = median(Sn_study),
            Sn_lower = quantile(Sn_study, probs = 0.05),
            Sn_upper = quantile(Sn_study, probs = 0.95),
            Sn_change = ifelse((Sn_upper > 0 & Sn_lower < 0), 0.9, 1),
            ENSPIE_slope = median(ENSPIE_study),
            ENSPIE_lower = quantile(ENSPIE_study, probs = 0.05),
            ENSPIE_upper = quantile(ENSPIE_study, probs = 0.95),
...
btx_posterior_cor <- as.mcmc(btx_multi4_fit, combine_chains = TRUE, 
                            pars = "^cor") %>% 
  as_tibble()
...
btx_sigma_post <- as.mcmc(btx_multi4_fit, combine_chains = TRUE, 
                         pars = "^sd") %>% 
  as_tibble()
