...
# want to get SS-leveleffects
all_coef <- coef(PREDICTS_multi4_fit, robust = TRUE, probs = c(0.05, 0.95)) 
...
SS_LU_multi <- SS_LU_multi %>% 
  mutate(sizeS = ifelse(alphaS==1, 2, .5),
         ...
         componentChange_sign = ifelse(componentChange=='N & ENSPIE', sign(N) * sign(S_PIE),
                                       ifelse(componentChange=='S & N', sign(S) * sign(N),
                                              ifelse(componentChange=='Sn & N', sign(Sn) * sign(N),
                                                     ifelse(componentChange=='S & Sn', sign(S) * sign(Sn),
                                                            ifelse(componentChange=='S & ENSPIE', sign(S) * sign(S_PIE),
                                                                   ifelse(componentChange=='Sn & ENSPIE', sign(Sn) * sign(S_PIE),
                                                                          ifelse(componentChange=='Sn, N & ENSPIE', sign(Sn) * sign(N) * sign(S_PIE),
                                                                                 ifelse(componentChange=='S, Sn, N & ENSPIE', sign(S) * sign(Sn) * sign(N) * sign(S_PIE),
                                                                                        ifelse(componentChange=='S, N & ENSPIE', sign(S) * sign(N) * sign(S_PIE), NA)))))))))
  )
...
# model estimates SS - LU combinations that are not in the data
predicts_dat <- PREDICTS_multi4_fit$data %>%
  as_tibble() %>% 
  filter(!is.na(Sn_5)) %>% 
  distinct(SS, LU)
  
predicts_dat$LU <- factor(predicts_dat$LU,
                   levels = c('PriVeg',
                              'Mature_SecVeg',      
                              'Intermediate_SecVeg',
                              'Young_SecVeg',
                   ...
                   labels = c('Primary vegetation',
                              ...
                              'Pasture',
                              'Urban'))
# filter to get to levels in the data only
predicts_filter <- predicts_dat %>% 
  unite(filter, c(SS, LU), remove = F)

SS_LU_multi_filtered <- SS_LU_multi %>% 
  unite(filter, c(SS, LU), remove = F) %>% 
  filter(filter %in% predicts_filter$filter)

save(SS_LU_multi, SS_LU_multi_filtered,
     PREDICTS_SS_posterior, predicts_cor_long,
